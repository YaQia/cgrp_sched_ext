...
struct scx_scheduler {
	...
	/*
	 * idle_mask_cpu and idle_mask_smt should be stored differently. Otherwise
	 * user might get erroneous updated mask.
	 */
	cpumask_var_t		idle_mask_cpu CL_ALIGNED_IF_ONSTACK;
	cpumask_var_t		idle_mask_smt CL_ALIGNED_IF_ONSTACK;
	...
	/* Currently we only support [0, 1]. */
	u8			scx_sched_idx:1;
} dummy_sched = {.scx_sched_idx = 0};
...
/*
 * Usually, curr_sched is the current task's scheduler, but it can
 * be temporarily set to other schedulers for their operations.
 */
...
// #define GET_SCX_HAS_OP(sch_idx, op_idx) scx_has_op[sch_idx * SCX_OPI_END + op_idx]
//
...
static struct scx_dsp_ctx __percpu *tmp_scx_dsp_ctx;
...
find_global_dsq(struct scx_scheduler *const sched, struct task_struct *p)
{
	return sched->global_dsqs[cpu_to_node(task_cpu(p))];
}

static struct scx_dispatch_q *
find_user_dsq(struct scx_scheduler *const sched, u64 dsq_id)
{
	return rhashtable_lookup_fast(&sched->dsq_hash,
				      &dsq_id, dsq_hash_params);
}
...
({										\
	__typeof__(curr_sched->scx_ops.op(args)) __ret;				\
	if (mask) {								\
		scx_kf_allow(mask);						\
		__ret = curr_sched->scx_ops.op(args);				\
		scx_kf_disallow(mask);						\
	} else {								\
		__ret = curr_sched->scx_ops.op(args);				\
	}									\
...
({										\
	__typeof__(curr_sched->scx_ops.op(task, ##args)) __ret;			\
	BUILD_BUG_ON((mask) & ~__SCX_KF_TERMINAL);				\
	current->scx.kf_tasks[0] = task;					\
...
({										\
	__typeof__(curr_sched->scx_ops.op(task0, task1, ##args)) __ret;		\
	BUILD_BUG_ON((mask) & ~__SCX_KF_TERMINAL);				\
...
static enum scx_ops_enable_state scx_ops_set_enable_state
(struct scx_scheduler *const sched, enum scx_ops_enable_state to)
{
	return atomic_xchg(&sched->scx_ops_enable_state_var, to);
}

static bool scx_ops_tryset_enable_state(struct scx_scheduler *const sched,
					enum scx_ops_enable_state to,
					enum scx_ops_enable_state from)
{
	int from_v = from;

	return atomic_try_cmpxchg(&sched->scx_ops_enable_state_var,
				  &from_v, to);
}
 ...
 */
static bool ops_cpu_valid(const struct scx_scheduler *sched, s32 cpu, const char *where)
{
	if (likely(cpu >= 0 && cpu < nr_cpu_ids && 
		   cpumask_test_cpu(cpu, sched->avail_mask))) {
		return true;
	} else {
...
#ifdef CONFIG_SCHED_CORE
	if (SCX_SCHED_HAS_OP(sched, core_sched_before))
			     ...
			     u64 enq_flags)
{
	...
	if (!is_local) {
		...
		if (unlikely(dsq->id == SCX_DSQ_INVALID)) {
			...
			dsq = find_global_dsq(task_group(p)->sched, p);
			raw_spin_lock(&dsq->lock);
		}
	}
	...
	if (is_local) {
		struct scx_scheduler *sched = task_group(p)->sched;
		struct rq *rq = container_of(dsq, struct rq, 
					scx.local_dsq[sched->scx_sched_idx]);
		bool preempt = false;
...
{
	u8 sched_id = task_group(p)->sched->scx_sched_idx;
	struct scx_dispatch_q *dsq = p->scx.dsq;
	bool is_local = dsq == &rq->scx.local_dsq[sched_id];
...
{
	struct scx_scheduler *sched = task_group(p)->sched;
	u8 sched_id = sched->scx_sched_idx;
	struct scx_dispatch_q *dsq;

	if (dsq_id == SCX_DSQ_LOCAL)
		return &rq->scx.local_dsq[sched_id];

	if ((dsq_id & SCX_DSQ_LOCAL_ON) == SCX_DSQ_LOCAL_ON) {
		s32 cpu = dsq_id & SCX_DSQ_LOCAL_CPU_MASK;

		if (!ops_cpu_valid(sched, cpu, 
				   "in SCX_DSQ_LOCAL_ON dispatch verdict")) {
			return find_global_dsq(sched, p);
		}
		return &cpu_rq(cpu)->scx.local_dsq[sched_id];
	}

	if (dsq_id == SCX_DSQ_GLOBAL)
		dsq = find_global_dsq(sched, p);
	else
		dsq = find_user_dsq(sched, dsq_id);
...
static void direct_dispatch(struct task_struct *p, u64 enq_flags)
{
	 ...
	 */
	if (dsq->id == SCX_DSQ_LOCAL && dsq != &rq->scx.local_dsq[sched_id]) {
		unsigned long opss;
			    ...
			    int sticky_cpu)
{
	...
	if (!unlikely(sched->scx_ops_enq_exiting) &&
	    unlikely(p->flags & PF_EXITING))
		goto local;

	if (!SCX_SCHED_HAS_OP(sched, enqueue))
		goto global;
...
local_norefill:
	dispatch_enqueue
		(&rq->scx.local_dsq[sched->scx_sched_idx], p, enq_flags);
	return;

global:
	touch_core_sched(rq, p);	/* see the comment in local: */
	p->scx.slice = SCX_SLICE_DFL;
	dispatch_enqueue(find_global_dsq(sched, p), p, enq_flags);
}
...
static void enqueue_task_scx(struct rq *rq, struct task_struct *p, int enq_flags)
{
	...
	if (SCX_SCHED_HAS_OP(sched, runnable) && !task_on_rq_migrating(p)) {
		switch_this_cpu_curr_sched(sched);
...
static void ops_dequeue(struct task_struct *p, u64 deq_flags)
{
	...
	case SCX_OPSS_QUEUED:
		if (SCX_SCHED_HAS_OP(sched, dequeue)) {
			switch_this_cpu_curr_sched(sched);
...
static bool dequeue_task_scx(struct rq *rq, struct task_struct *p, int deq_flags)
{
	 ...
	 */
	if (SCX_SCHED_HAS_OP(sched, stopping) && task_current(rq, p)) {
		update_curr_scx(rq);
		switch_this_cpu_curr_sched(sched);
		SCX_CALL_OP_TASK(SCX_KF_REST, stopping, p, false);
	}

	if (SCX_SCHED_HAS_OP(sched, quiescent) && !task_on_rq_migrating(p)) {
		switch_this_cpu_curr_sched(sched);
...
static void yield_task_scx(struct rq *rq)
{
	...
	if (SCX_SCHED_HAS_OP(sched, yield)) {
		switch_this_cpu_curr_sched(sched);
...
static bool yield_to_task_scx(struct rq *rq, struct task_struct *to)
{
	...
	struct scx_scheduler *sched = task_group(from)->sched;
	int ret;

	if (SCX_SCHED_HAS_OP(sched, yield)) {
		switch_this_cpu_curr_sched(sched);
		ret = SCX_CALL_OP_2TASKS_RET(SCX_KF_REST, yield, from, to);
		return ret;
	} else {
		return false;
	}
...
{
	u8 sched_id = task_group(p)->sched->scx_sched_idx;
	struct scx_dispatch_q *dst_dsq = &dst_rq->scx.local_dsq[sched_id];
...
static bool consume_global_dsq(struct scx_scheduler *sched, struct rq *rq)
{
	int node = cpu_to_node(cpu_of(rq));

	return consume_dispatch_q(rq, sched->global_dsqs[node]);
}
				  ...
				  struct task_struct *p, u64 enq_flags)
{
	...
	struct rq *dst_rq = container_of
				(dst_dsq, struct rq, scx.local_dsq[sched_id]);
	...
	if (unlikely(!task_can_run_on_remote_rq(p, dst_rq, true))) {
		dispatch_enqueue(find_global_dsq(sched, p), p,
	...
	if (likely(p->scx.holding_cpu == raw_smp_processor_id()) &&
		...
		if (src_rq == dst_rq) {
			p->scx.holding_cpu = -1;
			dispatch_enqueue(&dst_rq->scx.local_dsq[sched_id], p,
					 enq_flags);
		} else {
...
static int balance_one(u8 sched_id, struct rq *rq, struct task_struct *prev)
{
	struct scx_dsp_ctx *dspc = this_cpu_ptr(scx_dsp_ctx);
	...
	if (!sched || scx_ops_enable_state(sched) != SCX_OPS_ENABLED)
		return false;

	rq->scx.flags |= SCX_RQ_IN_BALANCE;
	rq->scx.flags &= ~(SCX_RQ_BAL_PENDING | SCX_RQ_BAL_KEEP);

	if (unlikely(sched->scx_ops_cpu_preempt) &&
		 ...
		 */
		if (SCX_SCHED_HAS_OP(sched, cpu_acquire)) {
			switch_this_cpu_curr_sched(sched);
	...
	if (rq->scx.local_dsq[sched_id].nr)
		goto has_tasks;

	if (consume_global_dsq(sched, rq))
		goto has_tasks;

	if (!SCX_SCHED_HAS_OP(sched, dispatch) || scx_rq_bypassing(rq) 
	    || !scx_rq_online(rq))
		goto no_tasks;
...
no_tasks:
	 ...
	 */
	if ((prev->scx.flags & SCX_TASK_QUEUED) &&
	    (!unlikely(sched->scx_ops_enq_last) ||
	     scx_rq_bypassing(rq))) {
		rq->scx.flags |= SCX_RQ_BAL_KEEP;
		goto has_tasks;
	}
...
static void set_next_task_scx(struct rq *rq, struct task_struct *p, bool first)
{
	...
	if (SCX_SCHED_HAS_OP(sched, running) 
	    && (p->scx.flags & SCX_TASK_QUEUED)) {
		switch_this_cpu_curr_sched(sched);
...
static void switch_class(struct rq *rq, struct task_struct *prev,
			 struct task_struct *next)
{
	const struct sched_class *next_class = next->sched_class;
	const struct scx_scheduler *sched = task_group(prev)->sched;
#ifdef CONFIG_SMP
	/*
	...
	if (!rq->scx.cpu_released) {
		if (SCX_SCHED_HAS_OP(sched, cpu_release)) {
...
static void put_prev_task_scx(struct rq *rq, struct task_struct *p,
			      struct task_struct *next)
{
	struct scx_scheduler *const sched = task_group(p)->sched;
	update_curr_scx(rq);

	/* see dequeue_task_scx() on why we skip when !QUEUED */
	if (SCX_SCHED_HAS_OP(sched, stopping) 
	    && (p->scx.flags & SCX_TASK_QUEUED)) {
		switch_this_cpu_curr_sched(sched);
	...
	if (p->scx.flags & SCX_TASK_QUEUED) {
		...
		if (p->scx.slice && !scx_rq_bypassing(rq)) {
			dispatch_enqueue
				(&rq->scx.local_dsq[sched->scx_sched_idx],
				 p, SCX_ENQ_HEAD);
			return;
		}
		...
		if (sched_class_above(&ext_sched_class, next->sched_class)) {
			WARN_ON_ONCE(!unlikely(sched->scx_ops_enq_last));
...
static struct task_struct *first_local_task(u8 sched_id, struct rq *rq)
{
	return list_first_entry_or_null
		(&rq->scx.local_dsq[sched_id].list,
		 struct task_struct, scx.dsq_list.node);
}
...
bool scx_prio_less(const struct task_struct *a, const struct task_struct *b,
		   bool in_fi)
{
	struct scx_scheduler *sched = a->sched_task_group->sched;
	int ret;
	/*
	 ...
	 * verifier.
	 */
	if (sched == b->sched_task_group->sched 
	    && SCX_SCHED_HAS_OP(sched, core_sched_before) 
	    && !scx_rq_bypassing(task_rq(a))) {
		switch_this_cpu_curr_sched(sched);
		ret = SCX_CALL_OP_2TASKS_RET(SCX_KF_REST, core_sched_before,
			      ...
			      u64 wake_flags, bool *found)
{
	...
	*found = false;
	...
	cpumask_and(sched->idle_mask_cpu, idle_masks.cpu, sched->avail_mask);
	cpumask_and(sched->idle_mask_smt, idle_masks.smt, sched->avail_mask);
	/*
	...
	if ((wake_flags & SCX_WAKE_SYNC)
	    && !cpumask_empty(sched->idle_mask_cpu)
	    && !(current->flags & PF_EXITING)
	    && cpu_rq(cpu)->scx.local_dsq[sched->scx_sched_idx].nr == 0) {
	 ...
	 */
	if (sched_smt_active()) {
		if (cpumask_test_cpu(prev_cpu, sched->idle_mask_smt)
	...
	if (cpumask_test_cpu(prev_cpu, sched->avail_mask) && 
	    test_and_clear_cpu_idle(prev_cpu)) {
		cpu = prev_cpu;
		goto cpu_found;
	}
...
static int select_task_rq_scx(struct task_struct *p, int prev_cpu, int wake_flags)
{
	...
	if (SCX_SCHED_HAS_OP(sched, select_cpu) 
		...
		*ddsp_taskp = NULL;
		if (ops_cpu_valid(sched, cpu, "from ops.select_cpu()")) {
			return cpu;
		} else {
			return prev_cpu;
		}
				 ...
				 struct affinity_context *ac)
{
	...
	cpumask_and(&p->cpus_mask, &p->cpus_mask, sched->avail_mask);
	 ...
	 */
	if (SCX_SCHED_HAS_OP(sched, set_cpumask)) {
		switch_this_cpu_curr_sched(sched);
...
void __scx_update_idle(struct rq *rq, struct task_struct *p, bool idle)
{
	struct scx_scheduler *const sched = task_group(p)->sched;
	int cpu = cpu_of(rq);

	if (SCX_SCHED_HAS_OP(sched, update_idle) 
	    && !scx_rq_bypassing(rq)) {
		switch_this_cpu_curr_sched(sched);
		SCX_CALL_OP(SCX_KF_REST, update_idle, cpu_of(rq), idle);
		if (!unlikely(sched->scx_builtin_idle_enabled))
			return;
	}
...
static void handle_hotplug(struct rq *rq, bool online)
{
	...
	for (i = SCHED_MAX_IDX; i >= 0; i--) {
		...
		if (!scx_enabled() || !sched 
		    || scx_ops_enable_state(sched) != SCX_OPS_ENABLED)
			return;
...
{
	struct scx_scheduler *const sched = task_group(curr)->sched;
	update_curr_scx(rq);
	...
	if (scx_rq_bypassing(rq)) {
		curr->scx.slice = 0;
		touch_core_sched(rq, curr);
	} else if (SCX_SCHED_HAS_OP(sched, tick)) {
		switch_this_cpu_curr_sched(sched);
...
{
	int ret;
	struct scx_scheduler *const sched = tg->sched;

	p->scx.disallow = false;

	if (SCX_SCHED_HAS_OP(sched, init_task)) {
		...
		// if (sched != curr_sched) {
		// 	pr_warn("sched_ext: scx_ops_init_task "
		// 			"called with incorrect scheduler.\n");
		// 	ret = ops_sanitize_err("init_task", -EINVAL);
		// 	return ret;
		// }
		switch_this_cpu_curr_sched(sched);
...
static void scx_ops_enable_task(struct task_struct *p)
{
	 ...
	 * This will guarantee the list is the well sorted by time.
	...
	if (SCX_SCHED_HAS_OP(sched, enable)) {
		switch_this_cpu_curr_sched(sched);
		SCX_CALL_OP_TASK(SCX_KF_REST, enable, p);
	}
	scx_set_task_state(p, SCX_TASK_ENABLED);

	if (SCX_SCHED_HAS_OP(sched, set_weight)) {
		switch_this_cpu_curr_sched(sched);
...
static void scx_ops_exit_task(struct task_struct *p)
{
	...
	if (SCX_SCHED_HAS_OP(sched, exit_task)) {
		switch_this_cpu_curr_sched(sched);
...
{
	if (scx_ops_enable_state(tg->sched) == SCX_OPS_DISABLED ||
	    cgroup_warned_missing_weight)
		return;

	if ((tg->sched->scx_ops.flags & SCX_OPS_HAS_CGROUP_WEIGHT) 
	     || !tg->css.parent)
		return;

	pr_warn("sched_ext: \"%s\" does not implement cgroup cpu.weight\n",
		tg->sched->scx_ops.name);
...
int scx_tg_online(struct task_group *tg, struct task_group *parent_tg)
{
	...
	if (scx_enabled() && scx_cgroup_enabled) {
		if (SCX_SCHED_HAS_OP(sched, cgroup_init)) {
			struct scx_cgroup_init_args args =
...
void scx_tg_offline(struct task_group *tg)
{
	...
	if (scx_enabled() && SCX_SCHED_HAS_OP(sched, cgroup_exit) 
...
int scx_cgroup_can_attach(struct cgroup_taskset *tset)
{
	...
	cgroup_taskset_for_each(p, css, tset) {
		...
		sched = task_group(p)->sched;
		if (SCX_SCHED_HAS_OP(sched, cgroup_prep_move)) {
			switch_this_cpu_curr_sched(sched);
...
err:
	cgroup_taskset_for_each(p, css, tset) {
		sched = task_group(p)->sched;
		if (SCX_SCHED_HAS_OP(sched, cgroup_cancel_move) 
		    && p->scx.cgrp_moving_from) {
			switch_this_cpu_curr_sched(sched);
			SCX_CALL_OP(SCX_KF_UNLOCKED, cgroup_cancel_move,
...
void scx_move_task(struct task_struct *p)
{
	 ...
	 */
	if (SCX_SCHED_HAS_OP(sched, cgroup_move) 
	    && !WARN_ON_ONCE(!p->scx.cgrp_moving_from)) {
		switch_this_cpu_curr_sched(sched);
		SCX_CALL_OP_TASK(SCX_KF_UNLOCKED, cgroup_move, p,
				 p->scx.cgrp_moving_from, tg_cgrp(tg));
	}
	p->scx.cgrp_moving_from = NULL;
}
...
void scx_group_set_weight(struct task_group *tg, unsigned long weight)
{
	...
	if (scx_cgroup_enabled && tg->scx_weight != weight) {
		sched = tg->sched ?: &dummy_sched;
		if (SCX_SCHED_HAS_OP(sched, cgroup_set_weight)) {
			switch_this_cpu_curr_sched(sched);
...
static struct scx_dispatch_q *
create_dsq(struct scx_scheduler *sched, u64 dsq_id, int node)
{
	struct scx_dispatch_q *dsq;
	int ret;
	...
	ret = rhashtable_insert_fast(&sched->dsq_hash, &dsq->hash_node,
				     dsq_hash_params);
	if (ret) {
		kfree(dsq);
		return ERR_PTR(ret);
...
static void destroy_dsq(struct scx_scheduler *sched, u64 dsq_id)
{
	struct scx_dispatch_q *dsq;
	unsigned long flags;
	...
	if (rhashtable_remove_fast(&sched->dsq_hash, &dsq->hash_node,
				   dsq_hash_params))
		goto out_unlock_dsq;
...
static int scx_cgroup_init(struct scx_scheduler *sched)
{
	...
	pr_info("sched_ext: scx_ops_enable before rcu_read_lock.\n");
	/*
	...
	rcu_read_unlock();
	pr_info("sched_ext: scx_ops_enable after rcu_read_unlock.\n");
 ...
 */
bool task_should_scx(struct scx_scheduler *const sched, int policy)
{
	if (!scx_enabled() || !sched || 
	    unlikely(scx_ops_enable_state(sched) == SCX_OPS_DISABLING 
		     && sched_cnt == 1))
		return false;
...
again:
	...
	scx_cgroup_unlock();
	pr_info("disable_workfn: after task_group scheduler changed.\n");
	...
	rhashtable_walk_enter(&sched->dsq_hash, &rht_iter);
	do {
		rhashtable_walk_start(&rht_iter);

		while ((dsq = rhashtable_walk_next(&rht_iter)) && !IS_ERR(dsq))
			destroy_dsq(sched, dsq->id);
...
static void schedule_scx_ops_disable_work(struct scx_scheduler *const sched)
{
...
static void scx_ops_disable(struct scx_scheduler *sched, enum scx_exit_kind kind)
{
	...
	if (!sched) {
		while (true) {
			...
			if (!sched)
				break;
			if (WARN_ON_ONCE(kind == SCX_EXIT_NONE 
					 || kind == SCX_EXIT_DONE))
				kind = SCX_EXIT_ERROR;
			  ...
			  struct task_struct *p, char marker)
{
	...
	if (SCX_SCHED_HAS_OP(sched, dump_task)) {
		ops_dump_init(s, "    ");
		switch_this_cpu_curr_sched(sched);
...
static void scx_dump_state
(struct scx_scheduler *const sched, struct scx_exit_info *ei, size_t dump_len)
{
	...
	if (SCX_SCHED_HAS_OP(sched, dump)) {
		ops_dump_init(&s, "");
		switch_this_cpu_curr_sched(sched);
	...
	for_each_possible_cpu(cpu) {
		...
		if (idle && !SCX_SCHED_HAS_OP(sched, dump_cpu))
			goto next;
		...
		if (SCX_SCHED_HAS_OP(sched, dump_cpu)) {
			ops_dump_init(&ns, "  ");
			switch_this_cpu_curr_sched(sched);
...
static void scx_ops_error_irq_workfn(struct irq_work *irq_work)
{
	...
	if (ei->kind >= SCX_EXIT_ERROR)
		scx_dump_state(sched, ei, sched->scx_ops.exit_dump_len);
...
static int scx_sched_init(struct scx_scheduler *const sched, struct task_group *root_grp)
{
	int ret = 0, node, i;
...
#ifdef CONFIG_SMP
	BUG_ON(!alloc_cpumask_var(&sched->avail_mask, GFP_KERNEL));
	BUG_ON(!alloc_cpumask_var(&sched->idle_mask_cpu, GFP_KERNEL));
	BUG_ON(!alloc_cpumask_var(&sched->idle_mask_smt, GFP_KERNEL));

	struct cpuset *cs = container_of(root_grp->css.cgroup->subsys[cpuset_cgrp_id],
					 struct cpuset, css);
	// TODO: or cs->cpus_allowed
	...
	if (!sched->global_dsqs) {
		...
		for_each_node_state(node, N_POSSIBLE) {
			 ...
			 */
			if (!cpumask_intersects(node_mask, sched->avail_mask)) {
				dsqs[node] = NULL;
				continue;
			}
			struct scx_dispatch_q *dsq;
...
static int scx_ops_enable(struct sched_ext_ops *ops, struct bpf_link *link)
{
	...
	if (!dummy_sched.avail_mask) {
		...
		dummy_sched.scx_root_kobj = kzalloc(sizeof(struct kobject),
					       GFP_KERNEL);
		if (!dummy_sched.scx_root_kobj) {
			ret = -ENOMEM;
			goto err_unlock;
		}

		dummy_sched.scx_root_kobj->kset = scx_kset;
		ret = kobject_init_and_add(dummy_sched.scx_root_kobj, 
					   &scx_ktype, NULL, dummy_sched.scx_ops.name);
	...
	if (sched_cnt == 0 
	    && scx_ops_enable_state(&dummy_sched) != SCX_OPS_DISABLED) {
		ret = -EBUSY;
		goto err_sched;
	}
	...
	sched->scx_exit_info = alloc_exit_info(ops->exit_dump_len);
	...
	WARN_ON_ONCE(scx_ops_set_enable_state(sched, SCX_OPS_ENABLING) !=
		     SCX_OPS_DISABLED);

	WARN_ON_ONCE(sched_cnt == 0 && scx_ops_set_enable_state
		     (&dummy_sched, SCX_OPS_ENABLING) != SCX_OPS_DISABLED);

	atomic_set(&sched->scx_exit_kind, SCX_EXIT_NONE);
	if (sched_cnt == 0)
		atomic_set(&dummy_sched.scx_exit_kind, SCX_EXIT_NONE);
	...
	if (sched->scx_ops.init) {
		/*
		 * Temporarily change this CPU's curr_sched into sched,
		 * so that the calling of init can be handled correctly.
		 */
		switch_this_cpu_curr_sched(sched);
	...
	if (sched_cnt == 0) {
		scx_dsp_max_batch = ops->dispatch_max_batch 
				    ?: SCX_DSP_DFL_MAX_BATCH;
		WARN_ON_ONCE(scx_dsp_ctx);
		scx_dsp_ctx = __alloc_percpu(struct_size_t
				(struct scx_dsp_ctx, buf, scx_dsp_max_batch),
				__alignof__(struct scx_dsp_ctx));
		if (!scx_dsp_ctx) {
			ret = -ENOMEM;
			goto err_disable;
		}
	} else if (ops->dispatch_max_batch) {
		tmp_scx_dsp_ctx = __alloc_percpu(struct_size_t
				    (struct scx_dsp_ctx, buf, 
				     scx_dsp_max_batch +
		...
		for_each_possible_cpu(i) {
			void *src = per_cpu_ptr(scx_dsp_ctx, i);
			void *dst = per_cpu_ptr(tmp_scx_dsp_ctx, i);
			memcpy(dst, src, struct_size_t
						(struct scx_dsp_ctx,
						 buf, scx_dsp_max_batch));
		}
		free_percpu(scx_dsp_ctx);
		scx_dsp_ctx = tmp_scx_dsp_ctx;
		scx_dsp_max_batch += ops->dispatch_max_batch;
	}
	...
	if (ops->flags & SCX_OPS_ENQ_LAST)
		sched->scx_ops_enq_last = true;

	if (ops->flags & SCX_OPS_ENQ_EXITING)
		sched->scx_ops_enq_exiting = true;
	if (sched->scx_ops.cpu_acquire || sched->scx_ops.cpu_release)
		sched->scx_ops_cpu_preempt = true;

	if (!ops->update_idle || (ops->flags & SCX_OPS_KEEP_BUILTIN_IDLE)) {
		reset_idle_masks();
		sched->scx_builtin_idle_enabled = true;
	} else {
		sched->scx_builtin_idle_enabled = false;
	}
	...
	while ((p = scx_task_iter_next_locked(&sti))) {
		...
		if (sched_cnt >= 1 && task_group(p)->sched != sched) {
			put_task_struct(p);
			continue;
		}
	...
	scx_task_iter_start(&sti);
	while ((p = scx_task_iter_next_locked(&sti))) {
		struct scx_scheduler *const p_sched = task_group(p)->sched;
		const struct sched_class *old_class = p->sched_class;
		const struct sched_class *new_class =
			__setscheduler_class(p_sched, p->policy, p->prio);
		struct sched_enq_and_set_ctx ctx;

		if (sched_cnt >= 1 && p_sched != sched)
			continue;
		if (old_class != new_class && p->se.sched_delayed)
			dequeue_task(task_rq(p), p, DEQUEUE_SLEEP | DEQUEUE_DELAYED);
		...
		// 		p->pid, p->comm, scx_get_task_state(p));
	}
	scx_task_iter_stop(&sti);
	percpu_up_write(&scx_fork_rwsem);
	...
	if (!scx_ops_tryset_enable_state
			(sched, SCX_OPS_ENABLED, SCX_OPS_ENABLING)) {
		WARN_ON_ONCE(atomic_read
				(&sched->scx_exit_kind) == SCX_EXIT_NONE);
		goto err_disable;
	}

	if (sched_cnt == 0 && !scx_ops_tryset_enable_state
			(&dummy_sched, SCX_OPS_ENABLED, SCX_OPS_ENABLING)) {
		WARN_ON_ONCE(atomic_read
			     (&dummy_sched.scx_exit_kind) == SCX_EXIT_NONE);
		goto err_disable;
	}
			       ...
			       void *kdata, const void *udata)
{
	...
	case offsetof(struct sched_ext_ops, root_cgroup_path):
		ret = strscpy(ops->root_cgroup_path, 
...
static void sysrq_handle_sched_ext_dump(u8 key)
{
	...
	if (scx_enabled())
		scx_dump_state(&dummy_sched, &ei, 0);
}
...
{
	enum scx_ops_enable_state state = 
		scx_ops_enable_state(task_group(p)->sched);
	...
	if (copy_from_kernel_nofault(&class, &p->sched_class, sizeof(class)) ||
		...
		printk("%sSched_ext: %s (%s%s)", log_lvl, 
		       curr_sched->scx_ops.name, 
	...
	printk("%sSched_ext: %s (%s%s), task: runnable_at=%s",
	       log_lvl, curr_sched->scx_ops.name, 
	       scx_ops_enable_state_str[state], all, runnable_at_buf);
}
...
{
	if (!likely(curr_sched->scx_builtin_idle_enabled)) {
				  ...
				  u64 enq_flags)
{
	...
	if (dst_dsq->id == SCX_DSQ_LOCAL) {
		struct scx_scheduler *const sched = task_group(p)->sched;
		u8 sched_id = sched->scx_sched_idx;
		dst_rq = container_of(dst_dsq, struct rq,
				      scx.local_dsq[sched_id]);
		if (!task_can_run_on_remote_rq(p, dst_rq, true)) {
			dst_dsq = find_global_dsq(sched, p);
			dst_rq = src_rq;
		}
	} else {
...
__bpf_kfunc bool scx_bpf_consume(u64 dsq_id)
{
	...
	dsq = find_user_dsq(curr_sched, dsq_id);
	if (unlikely(!dsq)) {
...
__bpf_kfunc u32 scx_bpf_reenqueue_local(void)
{
	LIST_HEAD(tasks);
	u32 nr_enqueued = 0;
	u8 sched_id = curr_sched->scx_sched_idx;
	struct rq *rq;
	struct task_struct *p, *n;
	 ...
	 */
	list_for_each_entry_safe(p, n, &rq->scx.local_dsq[sched_id].list,
				 scx.dsq_list.node) {
		/*
...
__bpf_kfunc s32 scx_bpf_create_dsq(u64 dsq_id, s32 node)
{
	if (unlikely(node >= (int)nr_node_ids ||
		...
		return -EINVAL;
	return PTR_ERR_OR_ZERO(create_dsq(curr_sched, dsq_id, node));
}
...
__bpf_kfunc void scx_bpf_kick_cpu(s32 cpu, u64 flags)
{
	...
	if (!ops_cpu_valid(curr_sched, cpu, NULL))
		return;
...
{
	struct scx_dispatch_q *dsq;
	struct scx_scheduler *const sched = curr_sched;
	s32 ret;
	u8 sched_id = sched->scx_sched_idx;
	...
	if (dsq_id == SCX_DSQ_LOCAL) {
		ret = READ_ONCE(this_rq()->scx.local_dsq[sched_id].nr);
		goto out;
	} else if ((dsq_id & SCX_DSQ_LOCAL_ON) == SCX_DSQ_LOCAL_ON) {
		s32 cpu = dsq_id & SCX_DSQ_LOCAL_CPU_MASK;

		if (ops_cpu_valid(sched, cpu, NULL)) {
			ret = READ_ONCE
			      (cpu_rq(cpu)->scx.local_dsq[sched_id].nr);
			goto out;
		}
	} else {
		dsq = find_user_dsq(sched, dsq_id);
				     ...
				     u64 flags)
{
	...
	kit->dsq = find_user_dsq(curr_sched, dsq_id);
	if (!kit->dsq)
		return -ENOENT;
...
__bpf_kfunc const struct cpumask *scx_bpf_get_idle_cpumask(void)
{
	...
	cpumask_and(curr_sched->idle_mask_cpu, curr_sched->avail_mask,
		    idle_masks.cpu);
#ifdef CONFIG_SMP
	return curr_sched->idle_mask_cpu;
#else
	return cpu_none_mask;
#endif
}
...
{
	struct scx_scheduler *const sched = curr_sched;
	if (!likely(sched->scx_builtin_idle_enabled)) {
		scx_ops_error("built-in idle tracking is disabled");
		return cpu_none_mask;
	}
...
#ifdef CONFIG_SMP
	if (sched_smt_active()) {
		cpumask_and(sched->idle_mask_smt, sched->avail_mask,
			    idle_masks.smt);
		return sched->idle_mask_smt;
	} else {
		cpumask_and(sched->idle_mask_cpu, sched->avail_mask,
			    idle_masks.cpu);
		return sched->idle_mask_cpu;
	}
#else
	return cpu_none_mask;
#endif
				      ...
				      u64 flags)
{
	...
	/*
	 * Should not use GFP_KERNEL here. This function can be called
	 * from sleep disabled path.
	 */
	if (!alloc_cpumask_var(&temp_mask, GFP_ATOMIC)) {
		scx_ops_error("no memory for internal temp cpumask");
		return -ENOMEM;
	}
	cpumask_and(temp_mask, cpus_allowed, sched->avail_mask);
	if (!likely(sched->scx_builtin_idle_enabled)) {
		scx_ops_error("built-in idle tracking is disabled");
		return -EBUSY;
	}

	ret = scx_pick_idle_cpu(temp_mask, flags);
				     ...
				     u64 flags)
{
	...
	cpumask_and(temp_mask, cpus_allowed, sched->avail_mask);
	if (likely(sched->scx_builtin_idle_enabled)) {
		cpu = scx_pick_idle_cpu(temp_mask, flags);
		if (cpu >= 0) {
			free_cpumask_var(temp_mask);
			return cpu;
		}
	...
	cpu = cpumask_any_distribute(temp_mask);
	free_cpumask_var(temp_mask);
